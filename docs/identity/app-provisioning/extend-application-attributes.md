...
To configure the LCW extensibility workflow mapping type, make sure you have an LCW extensibility workflow that invokes an Azure Logic App to generate a value. The value generated by the Azure Logic App is then applied to a target attribute.


## Prerequisites

You will leverage a Azure Logic App, so ensure you have ready:
...
- At least the [Logic App Contributor](https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/integration#logic-app-contributor) role

You will leverage LCW's extensibility workflow to trigger the Azure Logic App, so ensure you have ready:
- An [Entra ID Governance license](~/id-governance/licensing-fundamentals.md)
- At least the [Lifecycle Workflows Administrator](~/identity/role-based-access-control/permissions-reference.md#lifecycle-workflows-administrator) role

A provisioning job where you will perform the attribute mappings, with:
...
An LCW extensibility workflow is a type of [Lifecycle Workflow](~/id-governance/what-are-lifecycle-workflows.md) that is distinct from the existing Joiner, Mover, and Leaver types.
...
Create a consumption-based Azure Logic App that outputs a single value that will be applied to a target attribute.
